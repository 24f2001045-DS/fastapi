...
하지만 브라우저에서 클라이언트가 볼 수 있게 하려는 사용자 정의 헤더가 있다면, [Starlette의 CORS 문서](cors.md)에 문서화된 `expose_headers` 매개변수를 사용해 CORS 설정([CORS (Cross-Origin Resource Sharing)](https://www.starlette.dev/middleware/#corsmiddleware))에 추가해야 합니다.
